...
if [[ ! $built ]]; then ARRAY=(█ ▓ ▒ ░ ─); c=0; array=();
	 readarray -t oarray < <(ls \
		 ...
		 *.fl[cf] | tr -d "'" | xargs -I{} basename "{}" | sed 's/.fl[cf]//g'); for (( i=0; i<"${#oarray[@]}"; i++ )); do figlet -f "${oarray[i]}" '' 2> /dev/null && array[c++]="${oarray[i]}"; done; echo "${array[@]}" > /tmp/array.fvd; built=true; else array=(`cat /tmp/array.fvd`); fi; for (( i=0; i<"${#array[@]}"; i++ )); do t="${array[i]}"; echo $t; figlet -f "$t" "$txt"; read -s -n1 n </dev/tty; [[ "$n" == "k" ]] && ((i-=2)); [[ "$n" == "y" ]] && break; done; f="${array[i]}"; IFS=''; figlet -f $f "$txt" | tee /tmp/f1 > /tmp/f0; cat /tmp/f0; j=1; sed 's/./&\n/g' /tmp/f0 | grep -vx '' | awk '!seen[$0]++' | while read line; do echo -e "What do you want to replace ($line) with?\n${ARRAY[\
